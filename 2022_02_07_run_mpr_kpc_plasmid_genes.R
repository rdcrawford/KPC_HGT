...
load( "../data/kpcPlGeneMat.rData" )
print( dim( kpcPlGeneMat ) )
...
cgTree     = unroot( multi2di( cgTree, random = FALSE ) )
...
MakeMatFromList = function( trGeneList )
{
  ...
  cat(
    "Making the matrix for:", nTrs, "transitions and", nGenes, "genes\n"
    )
...
nGenes       = colSums( kpcPlGeneMat )
kpcPlGeneMat = kpcPlGeneMat[ , nGenes > 1 ]

# For each tree, find the edge on which each gene in the input matrix
# was acquired or deleted. Output an edge x gene matrix.
trMat = GetTransitionEdges( cgTree, kpcPlGeneMat, "mpr", "2059860.3" )
...
print( pryr::mem_used() )
rData = "../data/2022_02_07_run_mpr_kpc_plasmid_genes.rData"
save( file = rData, list = ls() )

write.tree( cgTree, file = "../data/cgTree.tre" )

# ---- Plot the heat maps ------------------------------------------------------

figDir  = "../figures/"
figPath = paste0( figDir, "transition_gene_correlation_matrix_kpc_pls.tiff" )
PlotHeatmap( corrMat, hCl, clstrData, figPath, '' )

figPath = paste0( figDir, "transition_gene_distance_matrix_kpc_pls.tiff" )
